...
var minPathSum = function (grid) {
  ...
  return _minPathSum(grid, grid.length - 1, grid[0].length - 1, cache);
};

function _minPathSum(grid, i, j, cache) {
  ...
  if (i > 0 && j > 0) {
    cache[i][j] =
      cur +
      Math.min(
        _minPathSum(grid, i - 1, j, cache),
        _minPathSum(grid, i, j - 1, cache)
      );
  } else if (i > 0 && j === 0) {
    cache[i][j] = cur + _minPathSum(grid, i - 1, j, cache);
  } else if (i === 0 && j > 0) {
    cache[i][j] = cur + _minPathSum(grid, i, j - 1, cache);
  } else {
    cache[i][j] = cur;
  }
  return cache[i][j];
